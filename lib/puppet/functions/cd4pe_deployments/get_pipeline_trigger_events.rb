...
# @summary Search for all the pipeline triggers that have occurred with a given SHA
Puppet::Functions.create_function(:'cd4pe_deployments::get_pipeline_trigger_events') do
  # @param repo_name
  ...
  # @example Get pipeline trigger events for a repository named 'control-repo'
  #   $pipeline = get_pipeline_trigger_events('control-repo', '1mfxk3kc4ic1w0nny0zcq5l7cw', '47b552dc15448b4e306fcf8df320e5124b2cbd63')
  # @return [Hash] contains the results of the function
  #   See [README.md]() for information on the CD4PEFunctionResult hash format
  #   * result [Hash]:
  #     * `rows [Array] array of hashes with pipeline results.`
  #   * error [Hash] contains error information if any
  #
  dispatch :get_pipeline_trigger_events do
    required_param 'String', :repo_name
    required_param 'String', :pipeline_id
    required_param 'String', :commit_sha
  end

  def get_pipeline_trigger_events(repo_name, pipeline_id, commit_sha)
    ...
    when Net::HTTPSuccess
      ...
      PuppetX::Puppetlabs::CD4PEFunctionResult.create_result(response_body['rows'])
